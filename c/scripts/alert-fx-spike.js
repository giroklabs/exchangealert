...
(출처: ${sourceLabel})
💵 DXY 인덱스: ${state.dxy || '-'}
🇺🇸 미국 10년물 금리: ${state.us10y !== null && state.us10y !== undefined ? state.us10y.toFixed(2) + '%' : '-'}
📉 한미 금리차: ${state.yieldDiff !== null && state.yieldDiff !== undefined ? state.yieldDiff.toFixed(2) + '%p' : '-'}
🏢 외인 순매수: ${state.fnb !== null && state.fnb !== undefined ? state.fnb + '억원' : '-'}
😨 VIX 공포지수: ${state.vix || '-'}
...
async function main() {
    ...
    if (currentState.yieldDiff !== null && lastInfo.yieldDiff !== null && lastInfo.yieldDiff !== undefined) {
    ...
    // US 10Y Yield Trigger (4.8, 4.9, 5.0, 5.1, 5.2 돌파)
    if (currentState.us10y !== null && lastInfo.us10y !== null && lastInfo.us10y !== undefined) {
        const thresholds = [5.2, 5.1, 5.0, 4.9, 4.8];
        for (const th of thresholds) {
            if (lastInfo.us10y < th && currentState.us10y >= th) {
                triggers.push(`📈 미국 10년물 국채금리 ${th.toFixed(1)}% 돌파! 👉 ${currentState.us10y.toFixed(2)}%`);
                break;
            }
        }
    }
