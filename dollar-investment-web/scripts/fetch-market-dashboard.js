...
[분석 지침 (각 1~2문단, **250자 이내** 추천)]

1. [파트A: 원/달러 환율 분석]
   - **현재 환율 수준(원)**에서 시작하여, 데이터로 제공된 지지/저항(지지 __원, 저항 __원)을 명시하며 수치 기반으로 전망을 설명해 주세요.
   - 위 정량예측모델의 확률(상승 ${upProb}%)과 CONTEXT를 기반으로, 예상 도달 범위(Target) 혹은 조정 시 하단 지지선을 구체적인 수치로 언급해 주세요.
   - 분석 내용 중 해당 확률 데이터를 언급할 때는 "정량예측모델"이라는 명칭을 사용해 주세요.

2. [파트B: 코스피 지수 분석]
   - **현재 지수(pt)**에서 시작하여, 데이터로 제공된 지지/저항(지지 __pt, 저항 __pt)을 명시하며 수치 기반으로 전망을 설명해 주세요.
   - 위 정량예측모델의 KOSPI 확률(상승 ${kospiUpProb}%)을 기반으로, 단기 반등 목표치 혹은 하락 지지 구간을 몇 pt인지 구체적인 숫자로 예측해 주세요.
   - 분석 내용 중 해당 확률 데이터를 언급할 때는 "정량예측모델"이라는 명칭을 사용해 주세요.

3. [실전 투자 대응]
   - 위 수치들을 바탕으로, 현재 포지션 보유자와 신규 진입자에 대한 구체적인 행동 지침(예: __원 이상 시 매도, __pt 이하 시 분할 매수 등)을 간결하고 직관적으로 제시해 주세요.

출력 형식 (**다음 형식을 엄격히 준수**하며 마크다운 헤더(#) 기호 사용 금지):

[파트A: 원/달러 환율 분석]
(1~2문단의 분석, **250자 이내** 추천, 구체적 수치 포함)

[파트B: 코스피 지수 분석]
(1~2문단의 분석, **250자 이내** 추천, 구체적 수치 포함)

실전 투자 대응:
- 환율: (추세 및 대응, 80자 이내 추천, 수치 포함)
- 코스피: (추세 및 대응, 80자 이내 추천, 수치 포함)

톤과 스타일:
- 객관적이고 공식적인 리포트 톤 사용 (수치에 기반한 근거 제시).
- "가능성", "리스크", "주의점"을 함께 언급하여 신중함을 유지하세요.`;
