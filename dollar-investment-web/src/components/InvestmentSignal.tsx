...
export function InvestmentSignal({ signal, isLoading }: InvestmentSignalProps) {
  ...
  if (!signal) {
    return (
      <div className="bg-white rounded-lg shadow-md p-6">
        <h3 className="text-lg font-semibold text-gray-700 mb-4">투자 적합성 분석</h3>
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <div className="flex items-center gap-3 mb-6">
        <div
          className={`flex-1 p-4 rounded-lg ${
            signal.isSuitable ? 'bg-green-50 border-2 border-green-500' : 'bg-red-50 border-2 border-red-500'
          }`}
        >
          <div className="flex items-center gap-2">
            {signal.isSuitable ? (
              <>
                ...
                <span className="text-xl font-bold text-green-700">투자 시작 적합</span>
              </>
            ) : (
              <>
                <span className="text-2xl">⚠️</span>
                <span className="text-xl font-bold text-red-700">투자 시작 부적합</span>
              </>
            )}
          </div>
        </div>
      ...
      <h3 className="text-lg font-semibold text-gray-700 mb-4">투자 조건 체크리스트</h3>
      <div className="space-y-3">
        {conditions.map((condition, index) => (
          <div
            key={index}
            className={`flex items-start gap-3 p-3 rounded-lg ${
              condition.met ? 'bg-green-50' : 'bg-red-50'
            }`}
          >
            <span className="text-xl mt-0.5">{condition.met ? '✅' : '❌'}</span>
            <div className="flex-1">
              <p
                className={`font-medium ${condition.met ? 'text-green-700' : 'text-red-700'}`}
              >
                {condition.label}
              </p>
              {condition.detail && (
                <p className="text-sm text-gray-600 mt-1">{condition.detail}</p>
              )}
            </div>
          </div>
        ))}
