...
export function Tabs({ activeTab, onTabChange, tabs }: TabsProps) {
    ...
    return (
        <div className={`w-full flex ${theme === 'dark' ? 'bg-gray-800/80' : 'bg-white'}`}>
            {tabs.map((tab) => (
                <button
                    key={tab.id}
                    onClick={() => onTabChange(tab.id)}
                    className={`flex-1 flex items-center justify-center gap-2 px-4 py-4 font-bold transition-all duration-300 border-b-2 ${activeTab === tab.id
